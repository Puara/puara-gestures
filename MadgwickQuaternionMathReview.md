# MadgwickQuaternion.h Math Review

## Assessment

I've reviewed the math in `MadgwickQuaternion.h`. Here's my assessment:

### 9-DoF `update()` — correct ✅

The full MARG update follows the standard Madgwick 2010 paper faithfully:

- **Quaternion derivative from gyro** — correct quaternion kinematic equation: `q̇ = 1/2 q ⊗ ω`
- **Accel/mag normalization** — correct
- **Earth magnetic field reference** — correctly computes `h = q ⊗ m ⊗ q*` then extracts `b_x = sqrt(h_x^2 + h_y^2)` and `b_z = h_z`, removing magnetic declination
- **Gradient descent step** — the Jacobian-transpose × objective-function product for `s0..s3` matches the paper's equations
- **Quaternion integration** — standard first-order Euler: `q_{t+1} = q_t + q̇ ⋅ Δt`, then re-normalize

### 6-DoF `updateIMU()` — correct ✅

The IMU-only fallback (no magnetometer) also matches the paper's simplified gradient for gravity-only correction.

## Minor observations (not bugs)

- `_4bx` and `_4bz` are computed but unused — `MadgwickQuaternion.h:163-164` define `_4bx = 2.0 * _2bx` and `_4bz = 2.0 * _2bz`, but the gradient terms below use `_4bx` in `s1–s3` and `_4bz` in `s1–s2`. Actually, looking more carefully, they are used in `s1`, `s2`, `s3` — so this is fine. ✅

- `_2bx` and `_2bz` naming is misleading — they're computed as `sqrt(h_x^2 + h_y^2)` and `h_z` respectively, then immediately multiplied by `2.0`, so at that point they're actually `2 b_x` and `2 b_z`. The names are correct. ✅

- **Redundant zero-check** — the magnetometer all-zeros case is checked twice (once to fall back to IMU-only, then again in the combined guard). Harmless but redundant.

- **`invSqrt` uses `1.0/sqrt()`** — perfectly fine for double precision on desktop. On embedded you might consider the fast inverse-sqrt approximation, but for accuracy this is better.

## Verdict

The math is correct and faithfully implements the Madgwick AHRS algorithm. No errors found.

---

## Annex: Original `MadgwickQuaternion.h`
